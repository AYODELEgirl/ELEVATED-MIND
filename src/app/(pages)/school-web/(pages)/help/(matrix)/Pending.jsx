...
function Pending() {
  ...
  const handleViewClick = (item) => {
    router.push(`/school-web/help/${item}`);
  };
  return (
    <div>
          ...
          {table?.map((e, i) => {
            return (
              <tr key={i} className="border-b border-gray-300">
                ...
                <td>
                  <div className="text-[#344054] text-[12px] w-fit rounded-md px-2 py-1 bg-[#F2F4F7]">
                    {e.staus}
                  </div>{" "}
                </td>
                <td>
                  <div className="text-white text-[13px] bg-[#043259] w-fit px-3 py-2 rounded-lg cursor-pointer">
                    <p onClick={() => handleViewClick(e.id)}>View</p>
                  </div>
                </td>
              </tr>
            );
